...
import { Menu, X, Search, User, Briefcase, Calendar, Newspaper } from 'lucide-react'
...
const Header = () => {
  ...
  const navigation = [
    { name: 'Buscar Empresas', href: '/busca', icon: Search },
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <Link to="/" className="flex items-center space-x-2">
            <div className="bg-blue-600 text-white p-2 rounded-lg">
              <span className="font-bold text-xl">noPortal</span>
            </div>
          </Link>
                  ...
                  <DropdownMenuLabel className="font-normal">
                    <div className="flex flex-col space-y-1">
                      ...
                      <p className="text-xs leading-none text-muted-foreground">
                        {user?.role === 'admin' ? 'Administrador' : user?.role === 'advertiser' ? 'Anunciante' : 'Usuário'}
                      </p>
                    </div>
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator />
