...
BEGIN
    ...
    IF NEW.status IS DISTINCT FROM OLD.status THEN
        ...
        -- Define o nome padrão se não for encontrado (ex: ação disparada por trigger ou rpc sem auth)
        v_current_user_name := COALESCE(v_current_user_name, 'Sistema');
        v_current_user_role := COALESCE(v_current_user_role, 'system'); -- Usar 'system' para ações sem usuário logado

        -- Define a fonte da ação para inclusão na mensagem
